...
def CumulFeatures(packets, featureCount): # packets: sizes
    separateClassifier = True
    if (not any(packets) or len(packets)==0) and separateClassifier: return [0] * (featureCount+4)
    elif (not any(packets) or len(packets)==0) and not separateClassifier: return [0] * (featureCount+4)
    ...
    for packetsize in itertools.islice(packets, None):
        ...
        if packetsize > 0:
            ...
            if len(cum) == 0:
                ...
                total.append(total[-1] + abs(packetsize)) # incoming, outgoing 상관X,  절댓값만
                pos.append(pos[-1] + packetsize) # incoming
                neg.append(neg[-1] + 0)
        ...
        if packetsize < 0:
            ...
            if len(cum) == 0:
                ...
                pos.append(pos[-1] + 0)
                neg.append(neg[-1] + abs(packetsize)) # outgoing
    ...
    # print("[1]", len(inCount), len(outCount), len(outSize),len(inSize))
    ...
    if separateClassifier:
        ...
        posFeatures = numpy.interp(numpy.linspace(total[0], total[-1], int(featureCount / 2)), total, pos)
        negFeatures = numpy.interp(numpy.linspace(total[0], total[-1], int(featureCount / 2)), total, neg)
